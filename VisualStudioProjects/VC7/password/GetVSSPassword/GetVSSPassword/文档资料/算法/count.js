var __cc_sys_url = "http://www.dircity.com/count_center/"


var __cc_ip_expire = 20;
var __cc_debug = false;


/*
	UPMEN Count Center 6.5 count.js
	Copyright(r) UPMEN Studio (www.dircity.com)
	coding by Ben (Ben@dircity.com)
	2004-8-1
	
	Please don't remove these information.
	var __cc_uid = "username";
	var __cc_style = 1;
*/

function cc6_readCookie(name){var cookieValue="";var search=name+"=";if(document.cookie.length>0){offset=document.cookie.indexOf(search);if(offset!=-1){offset+=search.length;end=document.cookie.indexOf(";",offset);if(end==-1)end=document.cookie.length;cookieValue=unescape(document.cookie.substring(offset,end))}}return cookieValue;}
function cc6_writeCookie(name,value,minutes){var expire="";if(minutes!=null){expire=new Date((new Date()).getTime()+minutes*60000);expire="; expires="+expire.toGMTString();}document.cookie=name + "="+escape(value)+expire;}

var __cc_cookies_name = 'cc_7_visited_site_'+__cc_uid;
var __cc_visited = parseInt(cc6_readCookie(__cc_cookies_name));
if(isNaN(__cc_visited)){cc6_writeCookie(__cc_cookies_name,1);__cc_visited=0;}else{cc6_writeCookie(__cc_cookies_name,++__cc_visited,__cc_ip_expire);}
if(!cc6_readCookie(__cc_cookies_name)) __cc_visited=-1;

var __cc_cookies_name = 'cc_7_return_site_'+__cc_uid;
var __cc_return = parseInt(cc6_readCookie(__cc_cookies_name));
if(isNaN(__cc_return)){cc6_writeCookie(__cc_cookies_name,1,60*24*365);__cc_return=0;}else{cc6_writeCookie(__cc_cookies_name,++__cc_return,60*24*365);}
if(!cc6_readCookie(__cc_cookies_name)) __cc_return=-1;


if(typeof(__cc_style)=='undefined') __cc_style=20;
if(typeof(__cc_customdata)=='undefined') __cc_customdata='';
var __cc_page = escape(location.href);
var __cc_ref = escape(document.referrer);
var __cc_Color = screen.colorDepth;
var __cc_Language = (navigator.systemLanguage?navigator.systemLanguage:navigator.language);
var __cc_sSize = screen.width + '_' + screen.height;
var __cc_count = __cc_sys_url 
	+ "core/default.asp" + "?id=" + __cc_uid
	+ "&l=" + __cc_Language + "&t=" + __cc_style
	+ "&p=" + __cc_page + "&r=" + __cc_ref
	+ "&s=" + __cc_sSize + "&c=" + __cc_Color
	+ "&v=" + __cc_visited + "&y=" + __cc_style
	+ "&b=" + __cc_return  + "&d=" + __cc_customdata
	+ "&z=" + (new Date()).getTimezoneOffset()/60;
if(__cc_debug){
	document.write(__cc_count);
	document.write("<iframe src='" + __cc_count + "' width=100% height=100></iframe>");
}else document.write("<script src='" + __cc_count + "'><\/script>");

/* End of count.js */