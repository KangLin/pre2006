...
static long gpibWrite(const int ud, char *wrt) {
...
static char *gpibQuery(const int ud, char *wrt, ... ) {
...
unsigned long tempOpen(char *port, const int model) {
    return (unsigned long)com_open(port, model);
}

int tempClose(const unsigned long ud) {
    return com_close((HANDLE)ud);
}

double tempMeasTemp(const unsigned long ud) {
    char hdr[5];
    char seed[5];
    char *cksm;
    char pkt[10];
...
unsigned long tempSet(const unsigned long ud, const int setpoint) {
...
char *spaMeasPhaseNoise(const int ud, 
                                   const double center, 
...
char *spaMeasSpectralFlatness(const int ud, 
                                         const double center, 
                                         const double ref_level,
                                         const int reset) {
...
double spaMeasChannelPower(const int ud, 
                                      const double center, 
                                      const double span, 
                                      const int reset) {
	char power[64];
	...
	int err=0;
	double retval = -120;
	const double ref_level = 25;
	const double rbw = 100;
	const double vbw = 30;
	const double int_bw = 100;
	const int det_mode = 1;
	const int averages = 7;
	const int max_hold = 1; 
   ...
   // if (reset) rsp = atoi(gpibQuery(ud, "*RST;*OPC?", 5L));
    ...
    // change to channel frequency
    gpibWrite(ud, qq("CF %f;", center));
    gpibWrite(ud,"PWCH");    
	gpibWrite(ud, "SP 60MZ");         // span
	gpibWrite(ud, "WDX 60MZ");
    if (reset) {
        
                gpibWrite(ud,    "RB 100e3;VB 30e3;");     // resolution bandwidth
				gpibWrite(ud,  qq("RL %f",ref_level));
				//gpibWrite(ud,	(averages == 0 ?(char*)"APAVG OFF":qq("AG %u",averages)));
                gpibWrite(ud,    "CONTS;");                 // disable continuous sweep
                gpibWrite(ud,    "PS;"); // search for max
        
    }
	gpibWrite(ud,    "DET SMP;");                         // detector mode (sample)
	gpibWrite(ud,    qq("SWPCNT %u;AAVG ON",averages));              // average 7 times
	//gpibWrite(ud,    "DET POS;");                         // detector mode (peak)
	for (i = 0; i < 7; i++) {
		gpibWrite(ud,    "TS;");                   // trigger sweep
	}
	buf = gpibQuery(ud,"PWCH?",64L);
	if(buf)
	{
		strncpy(power,buf,strlen(buf));
		retval = atof(power);
		//printf("..%f..",retval);
	}
...
double spaMeasSpectralDensity(const int ud, 
                                         const double center, 
                                         const double ref_level,
                                         const int reset) {
...
char *spaMeasOutOfBandEm(const int ud, 
                                    const double ref_level,
                                    const int reset) {
...
char *spaGetModelNo(const int ud, 
                                         const int reset) {
...
//++JC++
double spaMeasHarmonics(const int ud,
                                        const double center,
                                        const double vbw,
                                        const double ref_level,
                                        const int reset) {
...
double spaMeasTelecEmission2(const int ud,
...
double spaMeasTelecSpurious(const int ud,
                                        const double freq_start,
                                        const double freq_stop,
                                        const double ref_level,
                                        const int reset) {
...
double spaMeasBandEdge(const int ud,
                                        const double freq_start,
                                        const double freq_stop,
                                        const double vbw,
                                        const double ref_level,
...
double spaMeasPpsdUnii(const int ud,
                                        const double center,
                                        const double span, 
                                        const double ref_level,
                                        const int reset) {
...
double spaMeasPpsdDts(const int ud,
                                        const double center,
                                        const double span, 
                                        const double ref_level,
                                        const int reset) {
...
double spaMeasEbw(const int ud,
                                        const double center,
                                        const double span, 
                                        const double ref_level,
                                        const int reset) {
...
/*MANLIB_API double spaMeasChPwr(const int ud)
{
  char power[32];
  double retval = -120;
  int i=0,err=0;
  ...
  gpibWrite(ud,"PWCH");
  Sleep(100);
  buf = gpibQuery(ud,"PWCH?",32);
  printf("PWCH:%s",buf);
  if(strlen(buf)<=32)
  {
	  strncpy(power,buf,10);
	  // check for garbage
...
double spaMeasPkPwr(const int ud,
                                        const double center,
                                        const double vbw, 
                                        const double ebw, 
                                        const double ref_level,
...
double pmMeasPeakPower(const int ud, const int reset) {
...
// specified threshold
char *spaMeasTxSpurs(const int ud, 
                                   const double freq_start, 
                                   const double freq_stop,
...
double spaMeasPkAvgRatio(const int ud, 
                                    const double center, 
                                    const double ref_level,
                                    const int reset) {
...
double spaMeasFreqDev(const int ud, 
                                 const double center, 
                                 const double ref_level,
                                 const int reset) {
...
char *spaMeasTxSpuriousEm(const int ud, 
                                     const double ref_level,
                                     const int reset) {
...
char *spaMeasTxSpuriousEmLite(const int ud, 
                                         const double ref_level,
                                         const int reset) {
...
double spaMeasTxPwrDev(const int ud, 
                                  const double center, 
                                  const double ref_level,
                                  const int reset) {
...
char *spaMeasRxSpuriousEm(const int ud, 
                                     const double ref_level,
                                     const int reset) {
...
char *spaMeasRxSpuriousEmLite(const int ud, 
                                         const double ref_level,
                                         const int reset) {
...
double spaMeasOBW(const int ud, 
                             const double center, 
                             const double span,
                             const double ref_level,
                             const int reset) {
...
int spaMeasSpectralMask11b(const int ud,
    ...
    switch(spa_model) {
        ...
        case SPA_E4404B :
            ...
            gpibWrite(ud,    ":AVER:COUN 10;STATE 1");              // average 10 times
            gpibWrite(ud,    ":DET SAMP;");                         // detector mode (sample)
            //gpibWrite(ud,    ":DET POS;");                         // detector mode (peak)
            gpibWrite(ud,    ":INIT:IMM;*WAI;");                   // trigger sweep
...
int spaMeasPHSSpectralMask(const int ud,
                                   const double center,
                                   const double span,
                                   const int reset,
                                   const int verbose,
...
*******************************************************************************/
char *spaMeasRestrictedBand (const int ud,
                                        const double center,
                                        const double span,
