...
{	
	...
	TRACE(_T("%d:%s:%s\n"), uSeq, _T("开始发送"), szIP);
	if(SOCKET_ERROR == m_Sock.SendTo(&head, sizeof(head), uPort, szIP))
	{
		m_plog->Message(_T("发送数据出错。"));
		return 5;
	} // 结束 if(SOCKET_ERROR == m_Sock.SendTo(&head, sizeof(head), uPort, szIP))
	...
	while(1)
	{
		...
		TRACE(_T("%d:%s:%s\n"), i, _T("开始接收"), szIP);
		if(SOCKET_ERROR == m_Sock.Receive(buf, sizeof(buf)))
		{
			...
			return 2;
		} // 结束 if(SOCKET_ERROR == m_Sock.Receive(buf, sizeof(buf)))
