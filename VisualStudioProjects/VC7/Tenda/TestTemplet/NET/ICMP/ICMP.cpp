...
DWORD CICMP::ProccessICMP(char * buf, ICMPHeader * pICMPHead)
{
	...
	ucCopy = ucPacketSize - ucHeaderSize - ICMPHeaderLength;

	memcpy(&icmpHead, buf + ucHeaderSize, ICMPHeaderLength);
	if(pICMPHead)
	{
		memcpy(pICMPHead, buf + ucHeaderSize, ICMPHeaderLength);
	} // 结束 if(pICMPHead)
